...
ConVar gCV_csm_message_loopback;

#define INVALID_MESSAGE_ID -1

#include "csm-core/api.sp"
...
void CreateConVars()
{
	gCV_csm_message_loopback = CreateConVar("csm_message_loopback", "0", "Server sending messages will recieve his own messages. For testing purposes.", _, true, 0.0, true, 1.0);
}
...
{
	char error[255];
	gH_DB = SQL_Connect("csm", true, error, sizeof(error));
	if (gH_DB == null)
	{
...
void DB_QueryMessages()
{
	...
	if (!gCV_csm_message_loopback.BoolValue)
	{
